...
右下の **「＋ 投稿」** ボタンから、写真・国名・料理名・コメントを入力して投稿できます。
ブラウザから GitHub に直接コミットするため、初回だけ GitHub の設定が必要です。

**投稿できるのはトークンを設定したあなただけ**です（トークンはコードに埋め込まれず、あなたの端末のブラウザにのみ保存されます。公開サイトの閲覧者は投稿できません）。

1. **アクセストークンを作る**（fine-grained PAT 推奨）
   - GitHub → Settings → Developer settings → **Fine-grained tokens** → Generate new token
   - **Repository access**: このリポジトリのみ
   - **Permissions → Repository permissions → Contents: Read and write**
   - 生成された `github_pat_...` をコピー
2. サイトの「＋ 投稿」→ **⚙ GitHub設定** を開き、ユーザー名・リポジトリ名・トークンを入力して保存
   （`https://<あなた>.github.io/<repo>/` で開いていれば、ユーザー名とリポジトリ名は自動入力されます）
3. 以降は写真＋国名などを入れて「投稿する」だけ。地図には即時反映され、数十秒で公開版にも反映されます。

> 画像はアップロード前にブラウザ側で自動リサイズ（長辺1600px, JPEG）されます。
> 共用・公共のPCではトークンを保存しないでください。
