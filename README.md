...
**世界すべての国の料理を、ぜんぶ食べる**企画の記録サイト。
国ごとの料理を食べたら、その国は「制覇」（場所は問わない）。
...
投稿方法は「サイトの投稿ボタン」と「Discord」の2つ。どちらも同じ `data/posts.json` に入ります。

### A. サイトから投稿する（管理者モード）

投稿UIは通常は非表示で、**一般の閲覧者には「＋投稿」ボタンも GitHub 設定も見えません**。
自分の端末で投稿できるようにするには、**一度だけ URL の末尾に `#admin` を付けて開きます**:

```
https://<あなた>.github.io/<repo>/#admin
```

これでその端末が「管理者モード」になり（`localStorage` に記録）、右下に **「＋ 投稿」** が出ます。以後は普通に開くだけでOK。解除は「＋投稿 → ⚙設定 →『この端末の管理者モードを解除』」。

初回だけ GitHub のアクセストークンが必要です:

1. **アクセストークンを作る**（fine-grained PAT 推奨。classic の `repo` スコープでも可）
   - GitHub → Settings → Developer settings → **Fine-grained tokens** → Generate new token
   - **Repository access**: このリポジトリのみ
   - **Permissions → Repository permissions → Contents** の右のドロップダウンを **Read and write** に
   - 生成された `github_pat_...` をコピー
2. サイトの「＋ 投稿」→ **⚙ 設定** を開き、トークンを入力して保存
   （`github.io` で開いていれば、ユーザー名・リポジトリ名は自動入力）
3. 以降は写真＋国名などを入れて「投稿する」だけ。地図に即時反映、数十秒で公開版にも反映されます。

**編集・削除**: 管理者モードでは、地図や一覧から国を開いたモーダルに各投稿の「編集／削除」ボタンが出ます。

> - 画像はアップロード前にブラウザ側で自動リサイズ（長辺1600px, JPEG）。iPhone/Android の HEIC/HEIF は自動で JPEG に変換します。
> - トークンはコードに埋め込まれず、その端末のブラウザにのみ保存されます。共用・公共のPCでは保存しないでください。

### ホーム画面に追加（アプリ化 / PWA）

ネイティブアプリのようにホーム画面に置けます。
- **Android (Chrome)**: メニュー（︙）→「アプリをインストール」/「ホーム画面に追加」
- **iPhone (Safari)**: 共有ボタン →「ホーム画面に追加」

アイコンから全画面で起動し、オフラインでも地図が開きます（最後に見た内容をキャッシュ）。
...
| 項目 | どこから | 例 |
|---|---|---|
| **国名** | 1行目の `/` より前 | `イタリア`（日本語・英語 `Italy`・2文字コード `IT` いずれもOK） |
| **料理名** | 1行目の `/` より後（省略可） | `カルボナーラ` |
| **コメント** | 2行目以降（省略可） | `黒胡椒が効いてて…` |
| **日付** | 投稿日時から自動 | — |

- 区切り記号は `/` `／` `|` `｜` のいずれか。`/` が無ければ1行目全体が国名になります。
- 写真は最初の1枚を使用します。
- 国名が辞書に無い場合は `code: null` で保存され、地図には出ませんが投稿自体は失われません。`data/countries.json` に別名を足すか、2文字コードで投稿すれば解決します。
